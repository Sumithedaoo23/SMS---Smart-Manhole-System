document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentUserEmail = 'gaurang@example.com'; // Track the currently logged-in user
    const defaultProfile = userProfiles['gaurang@example.com'];

    // Function to set profile data in sidebar
    ...
    // Initial manholes for demonstration
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        const userProfile = userProfiles[enteredEmail];

        if (userProfile) {
            currentUserEmail = enteredEmail; // Set current user
            setProfileData(userProfile);
        } else {
            // If email not found, create a new profile for them
            userProfiles[enteredEmail] = {
                name: enteredEmail.split('@')[0], // Use part of email as name
                location: 'Unknown',
                phone: 'N/A',
                photo: 'https://via.placeholder.com/150' // Default photo
            };
            currentUserEmail = enteredEmail;
            setProfileData(userProfiles[enteredEmail]);
        }
    ...
    // Handle Add Manhole Form Submission
    addGutterForm.addEventListener('submit', (e) => {
        ...
        // Generate random data for new manhole
    ...
    // Manhole card creation function
    function createNewGutterCard(data) {
        const card = document.createElement('div');
        card.classList.add('dustbin-card', 'smart-gutter-card');
        ...
        card.innerHTML = `
            ...
            <div class="status-indicators-new">
                ...
                <div class="status-item-new">
                    <span class="status-label">Gas Levels</span>
                    <div class="gas-gauge">
        ...
        // Event listener for delete button on creation
    ...
    function animateIndicators(card) {
        ...
        mercuryEl.style.setProperty('--mercury-level', tempLevel + '%');
    ...
    editProfileBtn.addEventListener('click', () => {
        const currentProfile = userProfiles[currentUserEmail] || defaultProfile;
    ...
    editProfileForm.addEventListener('submit', (e) => {
        ...
        if (userProfiles[currentUserEmail]) {
            ...
            // This case handles if a new user logs in and then edits their profile
    ...
    // Initial setup: Hide dashboard and show login
    dashboardWrapper.style.display = 'none';
    loginContainer.style.display = 'block';
